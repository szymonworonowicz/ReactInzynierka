...
export const MessageService : IMessageService = {
    ...
    setMessageState: async(messages : UpdateMessageStateType[]) : Promise<boolean> => {
        const response = await apiClient.post<IBaseResponse<any>>('/messages/setMessageState',{messages});
