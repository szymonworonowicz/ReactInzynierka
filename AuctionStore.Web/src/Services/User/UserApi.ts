...
export const UserApi = {
  getUser: async (userId: string | null): Promise<IUserDto> => {
    const response = await apiClient.post<IBaseResponse<IUserDto>>(`users/`, {
      userId,
    });
  ...
  getAddresses: async (userId: string | null): Promise<Array<IAddress>> => {
    const response = await apiClient.post<IBaseResponse<Array<IAddress>>>(
      "users/getAddress",
      { userId }
    );

    if (response.data.success) {
      return new Promise<Array<IAddress>>((resolve) =>
  ...
  UpsertAddress: async (
    ...
    const response = await apiClient.post<IBaseResponse<IAddress>>(
      "users/upsertAddress",
      { ...address, userId }
    );
  ...
  getBankAccount: async (userId: string | null): Promise<IBankAccount> => {
    const response = await apiClient.post<IBaseResponse<IBankAccount>>(
  ...
  UpdateUserData: async (
    ...
    const response = await apiClient.post<IBaseResponse<IUserDto>>(
      "users/update",
      { ...data, userId }
    );
  ...
  getAddress: async (id: string): Promise<IAddress> => {
    const response = await apiClient.post<IBaseResponse<IAddress>>(
      "users/getOneAddress",
      { id }
    );
