...
namespace AuctionStore.Infrastructure.Services.Auth
{
    ...
    {
        public static JwtUserTokensDto GenerateUserTokens(string userId, string userLogin, List<Claim> customClaims,
            JwtOptions options)
        {
            ...
            var accessToken = DoGenerateToken(options.JwtTokenIssuer, options.JwtTokenSecretKey,
                options.AccessTokenExpiresInMinutes, userId, userLogin, claims);
            
            var refreshToken = DoGenerateToken(options.JwtTokenIssuer, options.JwtRefreshTokenSecretKey,
                options.RefreshTokenExpiresInMinutes.Value, userId, userLogin, claims);
        ...
        public static void GenerateUserTokensValidateOptions(JwtOptions options)
        {
            if (string.IsNullOrEmpty(options.JwtTokenSecretKey) || options.JwtTokenSecretKey.Length < 16)
            {
                throw new ArgumentException(
                    $"Parameter {nameof(options.JwtTokenSecretKey)} must have more than 15 chars");
            }
            ...
            {
                throw new ArgumentException(
                    $"Parameter {nameof(options.JwtRefreshTokenSecretKey)} must be different than {nameof(options.JwtTokenSecretKey)}");
            }

            if (options.AccessTokenExpiresInMinutes <= 0)
            {
                throw new ArgumentException(
                    $"Parameter {nameof(options.AccessTokenExpiresInMinutes)} must be more than 0");
            }

            if (!options.RefreshTokenExpiresInMinutes.HasValue || options.RefreshTokenExpiresInMinutes <= 0)
            {
                throw new ArgumentException(
                    $"Parameter {nameof(options.RefreshTokenExpiresInMinutes)} must be more than 0");
            }

            if (options.AccessTokenExpiresInMinutes >= options.RefreshTokenExpiresInMinutes.Value)
            {
                throw new ArgumentException(
                    $"Parameter {nameof(options.AccessTokenExpiresInMinutes)} must be higher than {nameof(options.RefreshTokenExpiresInMinutes)}");
            }
        }

        public static string DoGenerateToken(string JwtTokenIssuer, string secretKey, int expireInMinutes,
            string userId, string userLogin, List<Claim> customClaims)
        {
            var claims = new List<Claim>()
            {
        ...
        public static List<Claim> GetTokenClaims(string token, string jwtTokenIssuer, string secretKey)
        {
            ...
            try
            {
                ClaimsPrincipal tokenValid = tokenHandler.ValidateToken(token, tokenValidatorsParameters,
